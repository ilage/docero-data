    ...
    void checkAbstractRepositoryForPackage(String beanPkg) throws IOException {
        if (!abstractRepositoryCreated.contains(beanPkg)) {
            try (JavaClassWriter cf = new JavaClassWriter(environment, beanPkg + ".AbstractModuleRepository")) {
                ...
                cf.startBlock("public abstract class AbstractModuleRepository<T extends java.io.Serializable, C extends java.io.Serializable> " +
                        (spring ? " extends org.docero.data.AbstractSpringRepository<T,C>" : " extends org.docero.data.AbstractRepository<T,C>") +
                        " {");
                cf.startBlock("public C getKey(T bean){");
                {
                    cf.startBlock("try {");
                    cf.println("return (C) java.util.Arrays.stream(bean.getClass().getMethods()).filter(s -> s.getName().equals(\"getDDataBeanKey_\")).findFirst().get().invoke(bean,null);");
                    cf.endBlock("}");
                    cf.startBlock("catch (IllegalAccessException e) {");
                    cf.println("e.printStackTrace();");
                    cf.endBlock("}");
                    cf.startBlock("catch (java.lang.reflect.InvocationTargetException e) {");
                    cf.println("e.printStackTrace();");
                    cf.endBlock("}");
                    cf.println("return null;");
                }
                cf.endBlock("}");
                cf.println("abstract protected C save(T bean,org.docero.data.utils.UpdateOptions updateOptions, org.docero.data.DData dData,java.util.Set Set);");
                cf.endBlock("}");
            }
