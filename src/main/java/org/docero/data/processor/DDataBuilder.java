...
class DDataBuilder {
    ...
    void generateImplementation() throws IOException {
        ...
        try (JavaClassWriter cf = new JavaClassWriter(environment, basePackage + ".AbstractBean")) {
            cf.println("package " + basePackage + ";\n" +
                    ...
                    "public abstract class AbstractBean<T extends java.io.Serializable> " +
                    "implements org.docero.data.DDataComparable<T> {\n" +
                    "protected <P extends java.io.Serializable> P cached(Class<P> type, java.io.Serializable key) {\n" +
                    "return DData.cache(type, key);\n" +
                    "}\n" +
                    "protected <P extends java.io.Serializable> P remote(Class<P> type, java.lang.String func, java.io.Serializable... key) {\n" +
                    "return key == null || key.length == 0 || (key.length == 1 && key[0] == null) ? null : DData.remote(type, func, key);\n" +
                    "}\n" +
    ...
    void checkAbstractRepositoryForPackage(String beanPkg) throws IOException {
        if (!abstractRepositoryCreated.contains(beanPkg)) {
            try (JavaClassWriter cf = new JavaClassWriter(environment, beanPkg + ".AbstractModuleRepository")) {
                cf.println("package " + beanPkg + ";\n" +
                        "import org.docero.data.DData;\n" +
                        "/*\n" +
                        "Class generated by docero-data processor.\n" +
                        "*/\n" +
                        "public abstract class AbstractModuleRepository<T extends java.io.Serializable, C extends java.io.Serializable> " +
                        (spring ? " extends org.docero.data.AbstractSpringRepository<T,C>" : " extends org.docero.data.AbstractRepository<T,C>") +
                        " {\n" +
                        "public C getKey(T bean){\n" +
                        "        try {\n" +
                        "            return (C) java.util.Arrays.stream(bean.getClass().getMethods()).filter(s -> s.getName().equals(\"getDDataBeanKey_\")).findFirst().get().invoke(bean,null);\n" +
                        ...
                        "        return null;\n" +
                        "    }\n" +
                        "   abstract protected C save(T bean,org.docero.data.utils.UpdateOptions updateOptions, org.docero.data.DData dData,java.util.Set Set);\n" +
                        "}");
            }
