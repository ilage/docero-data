        ...
        */
        try (JavaClassWriter cf = new JavaClassWriter(environment, builder.basePackage + ".DDataModule")) {
            cf.println("package " + builder.basePackage + ";\n" +
                    ...
                    "import org.docero.data.utils.DDataException;");
            if (builder.spring)
                cf.println("import org.mybatis.spring.support.SqlSessionDaoSupport;");
            cf.println("/*\n" +
                    "Class generated by docero-data processor.\n" +
                    "*/\n" +
                    "public class DDataModule implements org.docero.data.utils.DDataModule {\n" +
                    "private final static java.util.HashMap<Class<?>,Class<?>> repositories = new java.util.HashMap<Class<?>,Class<?>>() {{");
            ...
            cf.println("}};\n" +
                    "public java.util.Map<Class,Class> getRepositories() {\n" +
                    "return java.util.Collections.unmodifiableMap(repositories);\n" +
                    "}\n" +
                    "public <T extends java.io.Serializable, C extends java.io.Serializable, R extends org.docero.data.DDataRepository<T, C>>" +
                    " Class<R> getBeanRepositoryInterface(Class<T> beanInterface) {\n" +
                    "return (Class<R>) repositories.get(beanInterface);\n" +
                    "}\n" +
                    "public <T extends java.io.Serializable,C extends java.io.Serializable> " +
                    "org.docero.data.DDataRepository<T,C> getBeanRepository(Class<T> beanInterface, SqlSessionFactory sqlSessionFactory) {\n" +
                    "org.docero.data.DDataRepository<T, C> r = null;\n" +
                    "Class<?> i = repositories.get(beanInterface);\n" +
                    "if(i != null) try {\n" +
                    "r = (org.docero.data.DDataRepository<T, C>) i.newInstance();");
            if (builder.spring)
                cf.println("((SqlSessionDaoSupport) r).setSqlSessionFactory(sqlSessionFactory);");
            cf.println("}\n" +
                    "catch (InstantiationException | IllegalAccessException e) {\n" +
                    "throw new RuntimeException(\"can,t instantiate \" + i.getName(), e);\n" +
                    "}\n" +
                    "return r;\n" +
                    "}\n" +
                    "public <R> R getRepositoryByInterface(Class<R> repositoryInterface, SqlSessionFactory sqlSessionFactory) {\n" +
                    "R r = null;\n" +
                    "Class<?> i = repositories.get(repositoryInterface);\n" +
                    "if(i != null) try {\n" +
                    "r = (R) i.newInstance();");
            if (builder.spring)
                cf.println("((SqlSessionDaoSupport) r).setSqlSessionFactory(sqlSessionFactory);");
            cf.println("return r;\n" +
                    ...
                    "return null;\n" +
                    "}\n" +
                    "    public <T extends java.io.Serializable, C extends java.io.Serializable> T save(\n" +
                    "            T t,\n" +
                    "            Class beanInterface,\n" +
                    ...
                    "    ) {\n" +
                    "        if (dData.getBeanRepository(beanInterface) instanceof AbstractModuleRepository) {\n" +
                    "            AbstractModuleRepository beanRepository = (AbstractModuleRepository) dData.getBeanRepository(beanInterface);\n" +
                    "            C key = (C) beanRepository.save(t, updateOptions, dData, new java.util.HashSet());\n" +
                    "            if (key != null)" +
                    "               return (T) beanRepository.get(key);\n" +
                    "            else" +
                    "               return t;" +
                    "        } else\n" +
                    "            return null;\n" +
            ...
            cf.println("\nprivate static final java.util.Map<Class<?>,Class<?>> implementations = " +
            ...
            cf.println("}};");
            cf.println("public java.util.Map<Class<?>, Class<?>> getImplementations() {");
            cf.println("return implementations;");
            cf.println("}");

            cf.println("");
            cf.println("private static final String[] cacheNames = new String[] {");
            ...
            if (environment.getElementUtils().getTypeElement("com.fasterxml.jackson.databind.JsonDeserializer") != null) {
                ...
                cf.println("*/");
                cf.println("private static final java.util.Map<Class<?>,com.fasterxml.jackson.databind.JsonDeserializer<?>> deserializers = " +
                ...
                for (String interfaceName : builder.beansByInterface.keySet()) {
                    ...
                    if (bean.abstractBean) {
                        ...
                        cf.println("this.put(" +
                                ...
                                interfaceName + ">() {");
                        cf.println("@Override");
                        cf.println("public " + interfaceName + " deserialize(com.fasterxml.jackson.core.JsonParser p, " +
                ...
                for (String table : discriminatedBeans.keySet()) {
                    ...
                    cf.println("if (root.has(\"" + prototype.discriminatorProperty.name + "\")) {");
                    {
                ...
                cf.println("}};");
                cf.println("public java.util.Map<Class<?>, com.fasterxml.jackson.databind.JsonDeserializer<?>> getDeserializers() {");
                cf.println("return deserializers;");
                cf.println("}");
            }

            cf.println("");
            cf.println("public String[] resources() {");
            cf.println("return new String[] {");
            for (DataRepositoryBuilder repository : builder.repositories)
            ...
            cf.println("");
            cf.println("public void register(org.docero.data.DData dData, org.docero.data.DDataDictionariesService ds) {");
        ...
        if (builder.spring) {
            try (JavaClassWriter cf = new JavaClassWriter(environment, builder.basePackage + ".DDataConfiguration")) {
                ...
                cf.println("public class DDataConfiguration {");

                for (DataRepositoryBuilder repository : builder.repositories) {
                    cf.println("@org.springframework.context.annotation.Bean");
                    cf.println("public " + repository.daoClassName + " " + repository.repositoryVariableName +
                            "(org.docero.data.DData dData) {");
                ...
                for (BatchRepositoryBuilder repository : builder.batchRepositories) {
                    ...
                    cf.println("@org.springframework.context.annotation.Bean");
                    cf.println("public " + daoInterfaceName + " " + methodName +
        ...
        try (JavaClassWriter cf = new JavaClassWriter(environment, packageName + ".ObjectFactory")) {
            ...
            for (DataBeanBuilder bean : pkgBeans) {
                ...
                cf.println("private final static QName _" + shortName +
                        ...
                        "public " + bean.getImplementationName() + " create" + shortName + "() {\n" +
                        "return new " + bean.getImplementationName() + "();\n" +
                        "}\n" +
                        "@XmlElementDecl(namespace = \"\", name = \"" + shortName + "\")\n" +
                        "public JAXBElement<" + bean.getImplementationName() + "> create" + shortName +
                        " (" + bean.getImplementationName() + " value) {\n" +
                        "return new JAXBElement<>(_" + shortName + "_QNAME, " +
